...
const AppletContainer: React.FC<OwnProps> = ({
    ...
    const state = useContext(appletInfo ? appletInfo.stateContext : DefaultStateContext)
    const dispatch = useContext(appletInfo ? appletInfo.dispatchContext : DefaultDispatchContext)
    ...
    return (
        <>
            ...
            {appletInfo ? (
                <>
                    <div>
                        <Slider
                            ...
                            onChange={(_, newValue: number | number[]) => dispatch({
                                action: "SetBallCount",
                                payload: newValue,
                            })}
                            min={5}
                ...
                <p>Oh no! {appletName} doesn't seem to be a valid Applet. :(</p>
            )}
        </>
    );
}
